...
class WordpieceTokenizer {
    public:
        WordpieceTokenizer(std::shared_ptr<Vocab> vocab, const std::wstring& unkToken = L"[UNK]", std::size_t maxInputCharsPerWord=200);
...
bool BasicTokenizer::isControl(const wchar_t& ch) const {
    if (ch== L'\t' || ch== L'\n' || ch== L'\r') return false;
